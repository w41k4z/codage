...
class Huffman:
    ...
    def encode(self, information: str):
        ...
        l = 0
        if len(encoded_information) % 8 != 0:
            l = 8 - (len(encoded_information) % 8)
            encoded_information += '0' * l
    ...
    def decode(self, encoded_file_path: str, file_key_path: str):
        ...
        with open(file_key_path, 'r') as file:
            content = file.read().removesuffix('\n')
            l = int(content[0])
            self.key = content[1:]
            file.close()
        ...
        while i < key_length:
            j = i + 1
            huffman_key = ''
            while j < key_length and self.key[j] != ' ':
                huffman_key += self.key[j]
                j += 1
            huffman_dict[huffman_key] = self.key[i]
            i = j + 1
        decoded_text = ''
        current_bits = ''
...
with open('output.txt', 'w') as file:
    file.write(huffman.decode('encoded', 'key'))
    file.close()